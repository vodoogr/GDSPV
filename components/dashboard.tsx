...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  const loadDashboardStats = async () => {
    ...
    try {
      ...
      // Obtener datos completos para calcular críticas, recientes y clientes únicos
      ...
      // Calcular clientes únicos con incidencias
      const clientesUnicosSet = new Set<string>()

      // Agregar clientes de incidencias principales
      if (incidencias && incidencias.length > 0) {
        incidencias.forEach((inc) => {
          // Buscar campos que puedan contener información del cliente
          const cliente = inc.cliente || inc.cod_cliente || inc.codigo_cliente || inc.client_id || inc.customer_id
          if (cliente && cliente.toString().trim() !== "") {
            clientesUnicosSet.add(cliente.toString().trim().toLowerCase())
          }
        })
      }

      // Agregar clientes de incidencias recibidas
      if (recibidas && recibidas.length > 0) {
        recibidas.forEach((rec) => {
          // Buscar campos que puedan contener información del cliente
          const cliente = rec.cliente || rec.cod_cliente || rec.codigo_cliente || rec.client_id || rec.customer_id
          if (cliente && cliente.toString().trim() !== "") {
            clientesUnicosSet.add(cliente.toString().trim().toLowerCase())
          }
        })
      ...
      console.log(`Clientes únicos calculados: ${clientesUnicos}`)
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: UserCheck,
      color: "bg-green-500",
      description: "Clientes con incidencias",
    },
    {
      title: "Críticas",
              ...
              <CardContent className={compactMode ? "p-4 pt-0" : ""}>
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-green-600 font-medium">{stats.clientesUnicos} clientes</span>
                  </div>
                </div>
              </CardContent>
            </CollapsibleContent>
      ...
      <Collapsible open={isInfoOpen} onOpenChange={setIsInfoOpen}>
        <Card>
          ...
          <CollapsibleContent>
            <CardContent className={compactMode ? "p-4 pt-0" : ""}>
              <div
                className={`grid gap-4 text-sm ${compactMode ? "grid-cols-1 md:grid-cols-4" : "grid-cols-1 md:grid-cols-4"}`}
              >
                ...
                <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                  <div className="font-medium text-green-900 dark:text-green-100 mb-1">Clientes Únicos</div>
                  <div className="text-green-700 dark:text-green-300">
                    Número único de clientes que tienen al menos una incidencia
                  </div>
                </div>
